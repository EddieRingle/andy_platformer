...
public class GameRenderer implements GLSurfaceView.Renderer {
    ...
    public void drawFPS(GL10 gl, float rightMargin) {
        ...
        if (m_msPerFrame > 0) {
            ...
            m_numericSprite.draw(gl, x, 0, width, height);
        }
    }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        float pWidth = _world.player.width;
        float halfPWidth = pWidth / 2.0f;
        float halfSWidth = width / 2.0f;
        ...
        for (int i = 0; i < len; i++) {
            if ((_world.platforms[i].xPos + _world.platforms[i].width >= offset)
                    && (_world.platforms[i].xPos <= offset + width)) {
        ...
        m_labels.beginDrawing(gl, width, height);
        float msPFX = width - m_labels.getWidth(m_labelFPS) - 1;
