...
public class GLQuad {
	...
	public GLQuad(World world, int w, int h, int texS, int texE)
	{
		...
		float vertices[] = {
				...
				(float)w, 0.0f, 0.0f, //bottom-right
				0.0f, (float)h, 0.0f, //top-left
				(float)w, (float)h, 0.0f, //top-left
		};
	...
	public void move(int w, int h)
	{
		...
		xPos += xVel;
		if (xPos < 0.0f || (xPos + this.width) > 2400.0f) {
			if (xPos < 0.0f)
				xPos = 0;
			else
				xPos -= xVel;
		...
		for (int i = 0; i < platformsLen; i++) {
			if (xPos < _world.platforms[i].xPos + _world.platforms[i].width && xPos + width > _world.platforms[i].xPos)
				if (yPos < _world.platforms[i].yPos + _world.platforms[i].height && yPos + height > _world.platforms[i].yPos)
					xPos -= xVel;
		}
		yPos += yVel;
		if (yPos + this.height < 0.0f) {
			this.alive = false;
		}
		for (int i = 0; i < platformsLen; i++) {
			if (yPos < _world.platforms[i].yPos + _world.platforms[i].height && yPos + height > _world.platforms[i].yPos)
		...
		/*
		for (int i = 0; i < platformsLen; i++) {
			if (xPos + this.width > _world.platforms[i].xPos && xPos < _world.platforms[i].xPos + _world.platforms[i].width) {
				if (yPos + this.height > _world.platforms[i].yPos && yPos < _world.platforms[i].yPos + _world.platforms[i].height) {
					xPos -= xVel;
					xRawVel = 0.0f;
				}
			}
	...
	public void draw(GL10 gl, int texOffset)
	{
		...
		gl.glTranslatef((float)this.xPos, (float)this.yPos, 1.0f);
